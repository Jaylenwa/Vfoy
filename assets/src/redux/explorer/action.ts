...
export const setFileList = (list: VfoyFile[]): ActionSetFileList => {
    return {
        type: "SET_FILE_LIST",
        list,
    };
...
export const setDirList = (list: VfoyFile[]): ActionSetDirList => {
    return {
        type: "SET_DIR_LIST",
        list,
    };
...
export const serverSideBatchDownload = (
    ...
    return (dispatch, getState): void => {
        ...
        const dirs: string[] = [],
            items: string[] = [];
        const fileSources: VfoyFile[] = selected.length ? selected : [...dirList, ...fileList];
                    ...
                    openGetSourceDialog(
                        response.data.length == 1
                            ...
                            : response.data
                                .map(
                                    (res) =>
                                        `${res.name}: ${res.url}${res.error ? res.error : ""
                                        }`
                                )
                                .join("\n")
                    )
