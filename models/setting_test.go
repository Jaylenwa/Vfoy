...
func TestGetSettingByName(t *testing.T) {
	...
	asserts.Equal("Vfoy", siteName)
	asserts.NoError(mock.ExpectationsWereMet())

	// 第二次查询应返回缓存内容
	siteNameCache := GetSettingByName("siteName")
	asserts.Equal("Vfoy", siteNameCache)
	asserts.NoError(mock.ExpectationsWereMet())
...
func TestGetSiteURL(t *testing.T) {
	...
	// 正常
	{
		...
		mock.ExpectQuery("SELECT(.+)").WithArgs("siteURL").WillReturnRows(sqlmock.NewRows([]string{"id", "value"}).AddRow(1, "https://drive.vfoy.org"))
		siteURL := GetSiteURL()
		asserts.NoError(mock.ExpectationsWereMet())
		asserts.Equal("https://drive.vfoy.org", siteURL.String())
	}
	...
	// 失败 返回默认值
	{
		...
		siteURL := GetSiteURL()
		asserts.NoError(mock.ExpectationsWereMet())
		asserts.Equal("https://vfoy.org", siteURL.String())
	}
}
